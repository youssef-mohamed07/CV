...
import { FaCode, FaDesktop, FaMobileAlt, FaUsers, FaSearchPlus, FaLanguage, FaGithub, FaLinkedin, FaTwitter, FaBrain, FaRocket } from 'react-icons/fa';
...
const Profile = () => {
  ...
  return (
    <section className="min-h-screen bg-gradient-to-br from-gray-100 via-gray-200 to-gray-300 dark:from-gray-900 dark:via-gray-800 dark:to-gray-700 py-20 px-4 sm:px-6 lg:px-8 flex flex-col items-center justify-center">
      <motion.h2
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-0">
          ...
          >
            ...
            <motion.h3
              className="text-4xl font-bold mb-8 text-gray-800 dark:text-white bg-clip-text text-transparent bg-gradient-to-r from-blue-500 to-purple-600"
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
            ...
            >
              {skills.map((skill, index) => (
                <SkillBadge key={index} icon={skill.icon} text={skill.text} color={skill.color} />
              ))}
            </motion.div>
          </motion.div>
        </div>
